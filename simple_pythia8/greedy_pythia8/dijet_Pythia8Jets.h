...
/*A selector class to increase the amount of b-jet events*/
class bJetSelector : public UserHooks
{
public:
    bJetSelector() {}
    virtual ~bJetSelector() {}

    /* In UserHooks this returns false */
    virtual bool canVetoProcessLevel() { return true; }

    /* Returns true, if the event is vetoed */

	int b_count = 0;

	/*b_max sets the MINIMUM amount of events with b quarks in the hard process of the Pythia.*/
	/*After generating b_max amount of b quark events, the generation will proceed as normal. */
	/*This also means that more b quark events are generated.*/
